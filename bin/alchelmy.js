...
(function($PS) {
  ...
  var renderRouter = function (application) {
      return function (fullPageModuleNames) {
          ...
          }), "          rerouting () = case parseLocation notFoundRoute location of", each(function (page_) {
              return block([ [ "                Route__", underbar(page_), " routeValue -> " ], [ "                    case ", page_, ".page.init (currentSession ()) location model.key routeValue of" ], [ "                        (initialModel, initialCmd) ->" ], [ "                            ( Model { model | state = State__", underbar(page_), " initialModel }" ], [ "                            , Cmd.map Msg__", underbar(page_), " initialCmd" ], [ "                            )" ] ]);
          }), "      in", "      case model.state of", each(function (page) {
              return block([ [ "             State__", underbar(page), " pmodel ->    " ], [ "               case parse ", page, ".page.route location of" ], [ "                  Just route -> case ", page, ".page.update (", page, ".page.onUrlChange location route) pmodel of" ], [ "                      (pageModel_, pageCmd) -> (Model { model | state = State__", underbar(page), " pageModel_ }, Cmd.map Msg__", underbar(page), " pageCmd)" ], [ "                  Nothing -> rerouting ()" ] ]);
          }), each(function (page) {
