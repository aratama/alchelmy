...
(function($PS) {
  ...
  var renderRouter = function (application) {
      return function (fullPageModuleNames) {
          ...
          }), "\x0aupdate : Route -> Msg -> Model -> ( Model, Cmd Msg )\x0aupdate notFoundRoute msg (Model model) =\x0a  case (msg, model.state) of\x0a    (UrlRequest urlRequest, _) ->\x0a          case model.state of\x0a", each(function (page) {
          ...
          }), "    (UrlChange location, _) ->", "      let ", "          currentSession : () -> Session", "          currentSession () = case model.state of", each(function (page) {
              return block([ [ "            State__", underbar(page), " pageModel ->", page, ".page.session pageModel " ] ]);
          }), "          rerouting cmd = case parseLocation notFoundRoute location of", each(function (page_) {
              return block([ [ "                Route__", underbar(page_), " routeValue -> " ], [ "                    case ", page_, ".page.init (currentSession ()) location model.key routeValue of" ], [ "                        (initialModel, initialCmd) ->" ], [ "                            ( Model { model | state = State__", underbar(page_), " initialModel }" ], [ "                            , Cmd.batch [cmd, Cmd.map Msg__", underbar(page_), " initialCmd]" ], [ "                            )" ] ]);
          }), "      in", "      case model.state of", each(function (page) {
              return block([ [ "             State__", underbar(page), " pmodel ->    " ], [ "                  case ", page, ".page.update (", page, ".page.onUrlChange location) pmodel of" ], [ "                      (pageModel_, pageCmd) -> case parse ", page, ".page.route location of " ], [ "                          Just _ -> (Model { model | state = State__", underbar(page), " pageModel_ }, Cmd.map Msg__", underbar(page), " pageCmd)" ], [ "                          _ -> rerouting (Cmd.map Msg__", underbar(page), " pageCmd)" ] ]);
          }), each(function (page) {
          ...
          }), "        ]", "\x0a\x0aparseLocation : Route -> Url -> Route\x0aparseLocation notFoundRoute location =\x0a    case parse matchers location of\x0a        Just route ->\x0a            route\x0a\x0a        Nothing -> \x0a            notFoundRoute \x0a      ", "\x0ainit : Route -> Flags -> Url -> Key -> ( Model, Cmd Msg )\x0ainit notFoundRoute flags location key =\x0a        case parseLocation notFoundRoute location of\x0a", each(function (page) {
          ...
          }), "\x0a\x0aprogram : { notFound : Route } -> Program Flags Model Msg\x0aprogram config =\x0a    application\x0a        { init = init config.notFound\x0a        , view = view\x0a        , update = update config.notFound\x0a        , subscriptions = subscriptions\x0a        , onUrlRequest = UrlRequest\x0a        , onUrlChange = UrlChange\x0a        }\x0a\x0a" ]);
      };
  };
